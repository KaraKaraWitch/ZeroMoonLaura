 ...
 * @help
 * 【更新履歴】
 * 1.25 2022/07/31 タグ情報管理の不具合修正
 *                 スキルのパーティクル表示設定で「user」に対応
 * 1.24 2022/06/24 敵のステートに関連する内部処理の変更
 * 1.23 2022/06/04 コアスクリプトv1.5.0のタイルサイズ変更対応
 * 1.22 2022/05/26 アニメーション画像使用時の不具合修正
 * 1.20 2022/03/20 exceedコマンドの不具合修正
 * 1.19 2022/02/05 exceedコマンドの不具合修正
 * 1.17 2022/01/09 iOSでの動作不具合修正
 * 1.16 2021/12/10 隊列入れ替え時のステート反映の不具合修正
 * 1.15 2021/12/06 戦闘中のメンバーの入れ替えに対応
 * 1.13 2021/10/31 不具合修正など
 * 1.09 2021/08/18 設定名の「/h」を自動補完
 *                 walk/startdashのデフォルトZ値をbelowに変更
 *                「weather:x,y」で視差スクロール対応
 *                 プラグイン設定「キャッシュ設定(β)」の実装
 * 1.07 2021/06/22 最大数制限が正しく変更されない不具合修正
 * 1.06 2021/06/01 ウィンドウ開閉時に白く表示される不具合への前提対応
 * 1.05 2021/04/22 内部処理の一部変更
 * 1.03 2021/04/14 screen
 * 1.02 2021/04/12 変数の制御文字に対応
 * 1.00 2021/04/10 初版。
 *
 * @param importLibrary
 * @text ライブラリインポート
 ...
 * @target MZ
 * @author Thirop
 * @plugindesc *Place below TRP_ParticleMZ_Preset
 * @help
 * [Update History]
 * 1.25 2022/07/31 Fixed a bug in tag information management.
 *                 Support for "user" target in particle display settings for skills.
 * 1.24 2022/06/24 Changes in internal processes related to enemy state handling
 * 1.23 2022/06/04 Core script v1.5.0 tile size change support
 * 1.22 2022/05/26 First Edition.
 *
 * @param importLibrary
 * @text Import Library
 * @desc ON/true to define the contents of pixi-particles.js in this plugin.To import separately, set to OFF / false.
 * @type boolean
 * @default true
 *
 * @param importFilter
 * @text import pixi-filters
 * @desc ON/true to define the contents of pixi-filters.js in this plugin.To import separately, set to OFF / false.
 * @type boolean
 * @default true
 *
 * @param systemParticles
 * @text Regular Particles
 * @desc Set mouse click/drag to display particles outside of the map and combat.
 * @type String[]
 * @default []
 *
 * @param commandName
 * @text Command Name
 * @desc Change command name when it clashes with another plugin game.Use comma to separate command names.
 * @default particle,パーティクル
 *
 * @param keepPictureOrder
 * @text Preserve picture stacking order
 * @desc When turned on, it will appear between each picture; when turned off, it will appear before and after all pictures. ON raises the likelihood of conflict.
 * @type boolean
 * @default false
 *
 * @param walkOffset
 * @text Offset during walk
 * @desc Displays the specified number of pixels in front of the character when walk is the subject.
 * @default 16
 * @type number
 *
 * @param dashOffset
 * @text Offset during startDash
 * @desc Displays the specified number of pixels in front of the character when startDash is the subject.
 * @default 16
 * @type number
 *
 * @param categoryClear
 ...
 *
 * @param clearCharacterOnMapChange
 * @text Clears subject when switching map
 * @desc Clears particle settings for events when switching map.
 * @default true
 * @type boolean
 * @parent categoryClear
 *
 * @param clearPartyOnMapChange
 * @text Clears party subject when switching map
 * @desc Clears particle setting for player/follower subject when switching map.
 * @default true
 * @type boolean
 * @parent categoryClear
 *
 * @param clearScreenOnMapChange
 * @text Clears Screen subject when switching map
 * @desc Clears particle setting for screen/weather/region when switching map.
 * @default true
 * @type boolean
 * @parent categoryClear
 *
 * @param clearBattleScreenOnEnd
 * @text Clears Battle Screen subject after the combat
 * @desc Clears the particle settings for battle/battleWeather subject after combat.
 * @default false
 * @type boolean
 * @parent categoryClear
 *
 * @param clearBattleCharaOnEnd
 * @text Clears battle character subject after the combat
 * @desc Clears the particle settings for party/enemy subject after combat.
 * @default true
 * @type boolean
 * @parent categoryClear
 *
 ...
 *
 * @param regionMargin
 * @text Off-screen width of region
 * @desc The number of off-screen margin tiles that generate particles when the target is region.(default2)
 * @type number
 * @default 2
 * @parent categoryPerformance
 *
 * @param outsideMargin
 * @text Off-screen width of character subject
 * @desc Number of off-screen blank tiles to generate particles when target is character.(default4,invalid when negative)
 * @default 6
 * @type number
 * @min -9999
 * @parent categoryPerformance
 *
 * @param maxParticles
 * @text Maximum number of particles
 * @desc Initial value of max number of particles on screen.(<0=no limit)When max,frequency etc will be adjusted.
 * @default 100000
 * @type number
 * @parent categoryPerformance
 *
 * @param useCache
 * @text Cache setting(beta)
 * @desc Reuse particles efficiently with ON/true. Please test the operation as this is a pilot function. 
 * @type boolean
 * @default false
 * @parent categoryPerformance
 *
 * @param categoryConflict
 * @text [Function invalidation/Conflict resolution]
 * @default ==============================
 *
 * @param disableState
 * @text Disables State Display
 * @desc Particle display for State will be disabled when turned ON.(Turn OFF when conflict occurs.)
 * @type boolean
 * @default false
 * @parent categoryConflict
 *
 * @param disableSkill
 * @text Disables Skill Display
 * @desc Particle display for Skill will be disabled when turned ON. (Turn OFF when conflict occurs.)
 * @type boolean
 * @default false
 * @parent categoryConflict
 *
 * @param disableRoute
 * @text Disables commands in travel route
 * @desc Command execution in travel route is disabled when turned ON.(Turn OFF when conflict occurs.)
 * @type boolean
 * @default false
 * @parent categoryConflict
 *
 * @param cacheBeforeTerminate
 * @text Caches before termination
 * @desc For resolving conflicts. Should be turned OFF in general.
 * @type boolean
 * @default false
 * @parent categoryConflict
 * 
 ...
 *
 * @param sceneTypes
 * @text Scene subject Registration
 * @desc Register class name when displaying it in a scene, such as the menu.
 * @type string[]
 * @default ["Scene_Menu-Scene_Save-Scene_Item-Scene_Equip-Scene_Actor-Scene_Skill-Scene_Status","Scene_Title","Scene_Load","Scene_Options","Scene_Shop","Scene_Gameover"]
 * @parent categorySenior
 *
 * @param noRewriteFunctions
 * @text Disables overwriting particle function
 * @desc Disables overwriting of some core functions from pixi-particle.js (for advanced users).
 * @type boolean
 * @default false
 * @parent categorySenior
 *
 ...
 *
 * @param displayCount
 * @text Displays number of particles
 * @desc Displays all particle numbers when ON(for debugging)
 * @type boolean
 * @default false
 * @parent categoryDebug
 *
 * @param errorLog
 * @text Error Indication
 * @desc Displays error when an incorrect ID or setting data name is specified.
 * @type boolean
 * @default true
 * @parent categoryDebug
 *
 * @param windowBugFix
 * @text Avoidance of window bugs
 * @desc Temporary fix for the blank name box bug when showing message with a transparent backdrop.
 * @type boolean
 * @default true
 * @parent categoryDebug
 *
 ...
 * @command set
 * @text set/display
 * @desc Displays particles
 *
 * @arg id
 * @text ManagementID
 * @desc The ManagementID must be unique. (Adding -EID at the end replaces EID with the eventID)
 * @default def
 *
 * @arg target
 * @text target
 ...
 * @arg name
 * @text Data name
 * @desc When set to def, the managementID is used as the data name.(When displaying the same data name multiple times, separate the managementIDs.)
 * @default def
 *
 * @arg z
 * @text Z value
 ...
 *
 * @command repeat
 * @text repeat/Repeat Setting
 * @desc Repeat/Playback setting
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 *
 * @arg interval
 * @text Interval
 * @desc Repeat immediately with 1. 0 to cancel repeat. 2 or more to repeat after the specified interval of frames.
 * @type number
 * @min 0
 * @default 1
 *
 ...
 *
 * @command on
 * @text on/Resume playback
 * @desc Unpause emitter
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 * @command clear
 * @text clear
 * @desc Delete the emitter that is currently in use.
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 * @desc When ON/true, deletes immediately without waiting for playback to finish.
 * @type boolean
 * @default false
 *
 * @arg delay
 ...
 *
 * @command update
 * @text update/Adjust parameters
 * @desc Adjust variables of the specified emitter's parameters.
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 * @arg key
 * @text Parameter name
 * @desc Name of the parameter to be adjusted, such as frequency, particlesPerWave, and pos.
 *
 * @arg values
 * @text Altering values (Connect with space)
 * @desc Use space to specify the value to be altered, such as for pos "100 200" would mean x=100, y=200
 * @arg delay
 * @text _Delay
 * @desc If set to 1 or more, the command will be executed after the specified frame.
 * @default 0
 ...
 *
 * @command animate
 * @text animate/Gradually change the parameter
 * @desc Gradually change the various parameters of specified emitter over time.
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 *
 * @arg duration
 * @text Number of frames required
 * @desc Number of frames required for parameter change.
 *
 * @arg key
 * @text Parameter name
 * @desc Name of the parameter to be adjusted, such as frequency, particlesPerWave, and pos.
 ...
 *
 * @command exceed
 * @text exceed/Advance time
 * @desc Manually advance the time of the specified emitter.
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 * @arg time
 * @text Skip (sec)
 * @desc Skip time(specify in seconds) 
 *
 * @arg delay
 * @text _Delay
 * @desc If set to 1 or more, the command will be executed after the specified frame.
 ...
 *
 * @command loop
 * @text loop/Screen loop
 * @desc Command for looping particles that are cut off at the top, bottom, left and right of the screen.
 *
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 *
 * @arg x
 * @text Horizontal margin(in pixels)/clear
 * @desc Horizontal margin(in pixels).[clear]to cancel loop setting.
 * @default def
 *
 * @arg y
 * @text Vertical margin(in pixels)
 * @desc Vertical margin(in pixels)
 * @default def
 *
 * @arg delay
 * @text _Delay
 ...
 * @desc Set a limit on the max number of overall displayed particles.
 *
 * @arg num
 * @text Max number
 * @desc Max number.0 for no limit. reset=reset to config setting value.
 * @default 0
 * 
 * @arg delay
 * @text _Delay
 ...
 * @desc Delete the filter set on the specified emitter.
 * 
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 * @command filterBlur
 * @text [filter]Blur
 * @desc Set Blur filter to specified emitter.
 * 
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 * @command filterGlow
 * @text [filter]Glow
 * @desc Set Glow filter to specified emitter.
 * 
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 *
 * @arg innerStretch
 * @text Inner glow level
 * @desc Inner glow level
 * @type number
 * @min 0
 * @default 0
 *
 * @arg outerStretch
 * @text Outer glow level
 * @desc Outer glow level
 * @type number
 * @min 0
 * @default 4
 *
 ...
 * @arg r
 * @text R value(Red)
 * @desc R value(Red).0~255
 * @type number
 * @default 255
 *
 * @arg g
 * @text G value(Green)
 * @desc G value(Green).0~255
 * @type number
 * @default 255
 *
 * @arg b
 * @text B value(Blue)
 * @desc B value(Blue).0~255
 * @type number
 * @default 255
 *
 * @arg delay
 ...
 *
 * @command filterRgbSplit
 * @text [filter]RGB Split 
 * @desc Set RGB split filter to specified emitter.
 * 
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 *
 * @arg rx
 * @text R(Red)X axis shift
 * @text R(Red)X axis shift
 * @type number
 * @default 3
 *
 * @arg ry
 * @text R(Red)Y axis shift
 * @text R(Red)Y axis shift
 * @type number
 * @default 3
 *
 * @arg gx
 * @text G(Green)X axis shift
 * @text G(Green)X axis shift
 * @type number
 * @default 3
 *
 * @arg gy
 * @text G(Green)Y axis shift
 * @text G(Green)Y axis shift
 * @type number
 * @default 3
 *
 * @arg bx
 * @text B(Blue)X axis shift
 * @text B(Blue)X axis shift
 * @type number
 * @default 3
 *
 * @arg by
 * @text B(Blue)Y axis shift
 * @text B(Blue)Y axis shift
 * @type number
 * @default 3
 *
 ...
 *
 * @command filterPixelate
 * @text [filter]Pixelate
 * @desc Set Pixelate filter to specified emitter.
 * 
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
 ...
 *
 * @command filterDisplacement
 * @text [filter]Displacement
 * @desc Set Displacement filter to specified emitter.
 * 
 * @arg targetId
 * @text Target managementID
 * @desc Target managementID
...
if(Game_Enemy.prototype.die===Game_BattlerBase.prototype.die
...
if(Game_Enemy.prototype.revive===Game_BattlerBase.prototype.revive
...
/* exceed
===================================*/
Game_Particle.prototype.particleExceed = function(id,arg,waitEmit=false){
	var targetIds = this.targetIds(id);
	var length = targetIds.length;
...
ParticleEmitter.prototype.initialize = function(id,data){
	...
	this._tileWidth = $gameMap ? $gameMap.tileWidth() : ($dataSystem.tileSize||48);
	this._tileHeight = $gameMap ? $gameMap.tileHeight() : ($dataSystem.tileSize||48);
	this._loopXThreshold = (this._isSceneMap && $gameMap.isLoopHorizontal()) ? $dataMap.width/2*this._tileWidth : 0;
	this._loopYThreshold = (this._isSceneMap && $gameMap.isLoopVertical()) ? $dataMap.height/2*this._tileHeight : 0;
	this._characterTargetMargin = isNaN(outsideMargin) ? -1 : this._tileWidth*Number(outsideMargin);
...
ParticleEmitter.prototype.initMembers = function(){
	...
	this._characterTargetMargin = isNaN(outsideMargin) ? -1 : 48*Number(outsideMargin);
	...
	if(this._isCharacterTarget && target){
		if(target instanceof Game_Event){
			if(target.event().meta.pMargin !== undefined){
				this._characterTargetMargin = this._tileWidth*Number(target.event().meta.pMargin);
			}
		}
	}
};
...
ParticleEmitter.prototype.frameForTileIndex = function(tileIndex){
	...
	return new Rectangle(this._tileWidth*col,this._tileHeight*row,this._tileWidth,this._tileHeight);
};
...
ParticleEmitter.prototype.update = function(){
	...
	if(!this._stop){
		...
		if(!this._freezed && data.exceed && data.exceed.id!==this._exceedId){
			if(!data.waitEmit || (this._emitter._poolFirst || this._emitter._activeParticlesFirst)){
...
var TRP_Emitter = /** @class */(function(_super){
	...
	function TRP_Emitter(particleParent, particleImages, config){
		...
		_this._tileWidth = $gameMap ? $gameMap.tileWidth() : ($dataSystem.tileSize||48);
		_this._tileHeight = $gameMap ? $gameMap.tileHeight() : ($dataSystem.tileSize||48);
	...
	TRP_Emitter.prototype._spawnRegion = function(validPositions,p,emitPosX,emitPosY){
	    ...
	    helperPoint.x = validPositions[rand]*this._tileWidth + Math.random() * this._tileWidth;
	    helperPoint.y = validPositions[rand+1]*this._tileHeight + Math.random() * this._tileHeight;
	    if (this.rotation !== 0){
