...
//Window_Selectable(一覧から項目を選択できるウィンドウ)を使用
//Window_DataListはWindow_Selectableを継承している(要はWindow_Selectableの機能を有している)
//superは継承元の機能を使用する
//全ての陣形情報の表示
class Window_DataList extends Window_Selectable {
    ...
    //maxItems:配列のlengthプロパティのようなものらしい
    //正常な動作であれば、index < maxItems()が常に成り立つ
    //再定義しない場合、この関数は0を返すようになり、描画されない
    maxItems() {
        ...
        if (this.innerRect.contains(x, y - this._titleSprite.bitmap.height)) {
            ...
            for (let i = 0; i < this.maxVisibleItems(); i++) {
                ...
                if (index < this.maxItems()) {
                    //itemRect:四角いカーソルや項目の表示座標に使用するパラメータ定義
...
class FormationData {
    static fromParam(id, param) {
        ...
        return new FormationData(id, param.FormationIndex, param.Name, param.IconIndex, param.Member, param.Description, positionDatas, param.MapId);
    }

    constructor(id, formationindex , name, iconIndex, member, description, positions, mapId) {
        this._id = id;
        this._formationindex = formationindex;
        this._name = name;
    ...
    get formationindex() { return this._formationindex; }
...
//Scene_MenuBase(全てのメニューシーンの親クラス)を使用
//メニュー画面に「陣形」を追加し、メニュー画面と陣形画面との接続を設定
class Scene_Formation extends Scene_MenuBase {
    ...
    //使用可能な陣形(選択中の陣形)欄でokを押した場合の処理
    //使用可能な陣形(選択中の陣形)→全ての陣形windowに遷移
    onEquipFormationListOk() {
        this.change_EquipFormationListWindow_To_HasFormationListWindow();
    }

    //使用可能な陣形(選択中の陣形)欄でcancelを押した場合の処理
    //使用可能な陣形(選択中の陣形)→メニュー画面windowに遷移
    onEquipFormationListCancel() {
        this.popScene();
    }

    //メニュー画面で陣形項目を選択した場合の処理
    //メニュー画面→陣形windowに遷移
    onEquipFormationListSelect() {
        this.changeFormation();
    }

    //使用可能な陣形(選択中の陣形)欄でshiftを押した場合の処理
    //使用可能な陣形(選択中の陣形)の中から装備する陣形を変更
    //※今回は使用しないので削除
    ...
    //全ての陣形から陣形を選択したときの処理
    //全ての陣形→選択した陣形を使用可能な陣形(選択中の陣形)に設定
...
class Window_FormationList extends Window_DataList {
    ...
    drawItem(index) {
        ...
        //全ての陣形を取得した場合、"-----"は表示させないようにするため、false処理削除
        //} else {
...
class Window_EquipFormationList extends Window_FormationList {
    ...
    //makeCommandList:使用可能な陣形(選択中の陣形)のコマンド作成
    makeCommandList() {
        //--- 21.10.24 add start
        //if (PartyMem !== $gameParty.members().length) {
        //--- 22.08.17 mod start
        //パーティ人数を4人から減らした場合、陣形が正しく反映されない不具合対応
        //if (PartyMem !== $gameParty.battleMembers().length) {
        //    for (let i = 0; i < FormationDatas.length; i++) {
        //        //if (FormationDatas[i].Member === $gameParty.members().length) {
        //        if (FormationDatas[i].Member === $gameParty.battleMembers().length) {
        //            $dataFormations.push(FormationData.fromParam(i, FormationDatas[i]));
        //        }
        //    if (PartyMem) {
        //        PartyMemFlg = 1;
        //    }
        //    //PartyMem = $gameParty.members().length;
        //    PartyMem = $gameParty.battleMembers().length;
        //} else if (!$dataFormations.length) {
        //    for (let i = 0; i < FormationDatas.length; i++) {
        //        if (FormationDatas[i].Member === $gameParty.members().length) {
        //            $dataFormations.push(FormationData.fromParam(i, FormationDatas[i]));
        //        }
        ...
        if ($gameParty.battleMembers().length < 4) {
            for (let i = 0; i < FormationDatas.length; i++) {
                if (FormationDatas[i].Member === 4) {
                    //$dataFormations.push(FormationData.fromParam(i, FormationDatas[i]));
                    $dataFormations.map(formationindex => $dataFormations[formationindex]);
                    break;
                }
            }
        } else {
        ...
        for (const formation of $gameParty.equipFormations()) {
            //--- 21.10.24 mod start
            if (formation !== null && formation !== undefined && formation !== ""){
    ...
    drawItem(index) {
        ...
        //↓使用可能な陣形(選択中の陣形)の枠を3→1に変更する為、削除
        //} else {
        //    this.drawText(Text.EmptySlot, rect.x, rect.y, rect.width, "center");
        }
    }
...
class Window_HasFormationList extends Window_FormationList {
    ...
    //makeCommandList:全ての陣形のコマンド作成
    makeCommandList() {
        for (const formation of $gameParty.hasFormations()) {
            //--- 21.10.24 mod start
            //全ての陣形に表示する陣形を登録
            //ただし、現在の陣形に設定されている陣形は表示させないようにする
            if (formation !== null && formation !== undefined && formation !== ""){
                this.addData(formation);
            }
            //this.addData(formation);
...
//Window_Selectable(一覧から項目を選択できるウィンドウ)を使用
//陣形の詳細情報(アクターの位置など)の表示
//Window_Selectable:コマンドカーソルの移動やスクロールを行うウィンドウ
class Window_FormationDetail extends Window_Selectable {
    ...
    //アクターの表示位置取得
    createActorSprites() {
        //maxBattleMembers:戦闘メンバーの参加人数取得
        //Sprite_MenuFormationActor：陣形表示で使用するアクターの画像を取得
        //sprite.startMotion:表示する画像のモーションを指定し、開始(今回はwalk(歩く))
        for (let i = 0; i < $gameParty.maxBattleMembers(); i++) {
            const actor = $gameParty.members()[i];
...
Game_Party.prototype.initialize = function() {
    ...
    this._equipFormationIds1 = new Array(NumEquipFormationSlots);
    this._equipFormationIds2 = new Array(NumEquipFormationSlots);
    this._equipFormationIds3 = new Array(NumEquipFormationSlots);
    this._equipFormationIds4 = new Array(NumEquipFormationSlots);
    this._equipFormationIds[0] = 0;
    this._equipFormationIds1[0] = 0;
    this._equipFormationIds2[0] = 0;
    this._equipFormationIds3[0] = 0;
    this._equipFormationIds4[0] = 0;
    this._hasFormationIds = [];
    this._hasFormationIds1 = [];
    this._hasFormationIds2 = [];
    this._hasFormationIds3 = [];
    this._hasFormationIds4 = [];
    };

//陣形システムプラグインで設定した陣形情報を取得
Game_Party.prototype.initFormations = function() {
    ...
    for (let i = 0; i < FormationDatas.length; i++) {
        ...
        //--- 22.08.17 mod start
        //パーティ人数を4人から減らした場合、陣形が正しく反映されない不具合対応
        //if (FormationDatas[i].Member === PartyMem) {    
        //    $dataFormations.push(FormationData.fromParam(i, FormationDatas[i]));
        //}
        if (FormationDatas[i].Member === 4) {    
            $dataFormations.push(FormationData.fromParam(i, FormationDatas[i]));
...
Game_Party.prototype.learnFormation = function(formationId) {
    ...
    //パーティ人数を4人から減らした場合、陣形が正しく反映されない不具合対応
        //switch (PartyMem){
        //case 1:
        //    if (this._equipFormationIds1.includes(formationId) || his._hasFormationIds1.includes(formationId)) return;
        //    this._hasFormationIds1.push(formationId);
        //    this._hasFormationIds = this._hasFormationIds1;
        //    break;
        //case 2:
        //    if (this._equipFormationIds.includes2(formationId) || this._hasFormationIds2.includes(formationId)) return;
        //    this._hasFormationIds2.push(formationId);
        //    this._hasFormationIds = this._hasFormationIds2;
        //    break;
        //case 3:
        //    if (this._equipFormationIds3.includes(formationId) || this._hasFormationIds3.includes(formationId)) return;
        //    this._hasFormationIds3.push(formationId);
        //    this._hasFormationIds = this._hasFormationIds3;
        //    break;
        //case 4:
        //    if (this._equipFormationIds4.includes(formationId) || this._hasFormationIds4.includes(formationId)) return;
        //    this._hasFormationIds4.push(formationId);
        //    this._hasFormationIds = this._hasFormationIds4;
        //    break;
        //}
        if (this._equipFormationIds4.includes(formationId) || this._hasFormationIds4.includes(formationId)) return;
        this._hasFormationIds4.push(formationId);
...
Game_Party.prototype.currentFormation = function() {
    ...
    //パーティ人数を4人から減らした場合、陣形が正しく反映されない不具合対応
        ...
        //switch (PartyMem){
        //case 1:
        //    this._equipFormationIds[0] = this._equipFormationIds1[0];
        //    break;
        //case 2:
        //    this._equipFormationIds[0] = this._equipFormationIds2[0];
        //    break;
        //case 3:
        //    this._equipFormationIds[0] = this._equipFormationIds3[0];
        //    break;
        //case 4:
        //    this._equipFormationIds[0] = this._equipFormationIds4[0];
        //    break;
        //}
    this._equipFormationIds[0] = this._equipFormationIds4[0];
    //--- 22.08.17 mod end
...
//使用可能な陣形(選択中の陣形)に表示する配列の設定
Game_Party.prototype.equipFormations = function() { 
    //--- 21.10.24 add start
    //if (PartyMemFlg && $dataFormations[this._equipFormationIds[0]].member !== PartyMem) {
    //--- 22.08.17 mod start
    //パーティ人数を4人から減らした場合、陣形が正しく反映されない不具合対応
        //if (PartyMemFlg) {
        //switch (PartyMem){
    ...
    //--- 22.08.17 mod end
    //--- 21.10.24 add end
    //↓使用可能な陣形(選択中の陣形)で配列にnull(空欄)が何故か存在するので削除する処理を追加
    for (let i = 0; i < this._equipFormationIds.length; i++){
        if (this._equipFormationIds[i] === null || this._equipFormationIds[i] === undefined || this._equipFormationIds[i] === ""){
            this._equipFormationIds.splice(i, 1);  // 削除
            if (i > 0) i--;
        }
    }
    //return this._equipFormationIds.map(id => $dataFormations[id]);
    return this._equipFormationIds.map(formationindex => $dataFormations[formationindex]);
};
...
Game_Party.prototype.hasFormations = function() {
    ...
    return this._hasFormationIds.map(formationindex => $dataFormations[formationindex]);
    //-- 21.11.23 mod end
};

//changeEquipFormationsの処理の流れ(現在の陣形を変更する場合の流れ)
//1.装備中の陣形スロット0の陣形IDを退避
//2.全ての陣形より選択した陣形IDを取得
//3.上記の陣形IDを装備中の陣形スロット0に設定
//4.スロットindex(0)がカレントスロットか判定
//5.全ての陣形から選択した陣形ID以外でthis._hasFormationIdsを作成し直す
//6.装備中の陣形に選択した陣形IDを登録
Game_Party.prototype.changeEquipFormations = function(slotIndex, targetFormation) {
    ...
    const hasFormationId = (targetFormation ? targetFormation.formationindex : null);
    //-- 21.11.23 mod end
    ...
    //switch (PartyMem){
    //    case 1:
    //        this._equipFormationIds1[0] = this._equipFormationIds[0];
    //        this._hasFormationIds1 = this._hasFormationIds;
    //        break;
    //    case 2:
    //        this._equipFormationIds2[0] = this._equipFormationIds[0];
    //        this._hasFormationIds2 = this._hasFormationIds;
    //        break;
    //    case 3:
    //        this._equipFormationIds3[0] = this._equipFormationIds[0];
    //        this._hasFormationIds3 = this._hasFormationIds;
    //        break;
    //    case 4:
    //        this._equipFormationIds4[0] = this._equipFormationIds[0];
    //        this._hasFormationIds4 = this._hasFormationIds;
    //        break;
    //}
    this._equipFormationIds4[0] = this._equipFormationIds[0];
    this._hasFormationIds4 = this._hasFormationIds;
...
class Window_BattleFormationList extends Window_FormationList {
    ...
    //maxCols:ウィンドウの横方向の要素数
    maxCols() {
        return 2;
    }

    //makeCommandList:陣形一覧？のコマンド作成
    makeCommandList() {
...
//Window_PartyCommand(戦闘画面でパーティ全体コマンド(逃げるなど)を表示するウィンドウ)を使用
...
//陣形習得
//アイテムから習得する(コモンイベントで習得する陣形を登録→アイテム使用時に登録したコモンイベントを呼び出す)
PluginManager.registerCommand(FormationSystemPluginName, "LearnFormation", args => {
    ...
    $gameVariables.setValue(params.VariableId,params.FormationId);
