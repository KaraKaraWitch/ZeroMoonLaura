...
/*:
 * @target MZ
 * @plugindesc [Ver1.3.0]Yet another menu screen layout
 ...
 *
 * -- 21.10.01. add start
 * @param playtime string
 * @parent playtime name
 * @text Playtime String
 * @desc prefix of playtime. It draws by system color.
 * @type string
 * @default Playtime:
 * -- 21.10.01. add end
 *
...
/*:ja
 * @target MZ
 * @plugindesc [Ver1.3.0]レイアウトの異なるメニュー画面
 * @author 神無月サスケ, ムノクラ
 *
 * @param allowWindowDisp
 * @text 背景未設定の時ウィンドウ表示？
 ...
 * @param playtime string
 * @parent playtime name
 * @text 「プレイ時間：」を意味するテキスト
 * @desc プレイ時間表示の際にシステムカラーで表示される文字列
 * @type string
 * @default プレイ時間:
 * -- 21.10.01. add end
 *
...
(() => {
  ...
  //-- 21.10.01 add start
  const playtimeString = parameters['playtime string'] || 'Playtime:';
  //-- 21.10.01 add end
  ...
  const isSceneNeedsAdjutGauge = () => {
    const sceneName = SceneManager._scene.constructor.name;
    return ["Scene_Menu", "Scene_Item", "Scene_Skill"].includes(sceneName);
  };

  const _Sprite_Gauge_bitmapWidth = Sprite_Gauge.prototype.bitmapWidth;
  Sprite_Gauge.prototype.bitmapWidth = function () {
    return isSceneNeedsAdjutGauge() ? 148 * 4 / realMaxColsMenuWnd() :
  ...
  Sprite_Name.prototype.bitmapWidth = function () {
    return isSceneNeedsAdjutGauge() ? 168 * 4 / realMaxColsMenuWnd() :
  ...
  Window_StatusBase.prototype.drawActorLevel2 = function (actor, x, y, width) {
    ...
    this.resetTextColor();
    this.drawText(actor.level, x, y, width, 'center');
  };
  ...
  //-- 21.10.01 add start
  const playTime = () => {
    return time = $gameSystem.playtimeText();
  };
  //-- 21.10.01 add end
  ...
  Window_MapNameAlt3.prototype.update = function () {
    ...
    if (playTime()) {
      ...
      const textWidth = this.textWidth(playtimeString) + this.itemPadding();
      const row = 730;
      const col = 4;
      this.drawText(playtimeString, row, col, this.width, 'left');
      this.resetTextColor();
      const orgX = row + textWidth;
      this.drawText(playTime(), orgX, col, this.width, 'left');
    }
    //-- 21.10.01 add end
  };
